...
在大多数研发团队，PRD → 任务拆解是一个高度人工、质量不稳定的环节：

| 痛点 | 典型影响 |
|------|---------|
| PRD 充斥自然语言歧义 | 研发理解偏差，需求返工 |
| Tech Lead 每份 PRD 需花 2-4 小时拆解 | 高价值人力浪费 |
| 歧义在开发中期才暴露 | 返工成本是需求阶段的 10x |
| 任务粒度、工时估算标准不统一 | Sprint 溢出，排期不准 |
...
**关键设计决策：**
- **Tool use（结构化输出）**：强制 Claude 返回 JSON schema 合规的结构，避免解析自然语言
