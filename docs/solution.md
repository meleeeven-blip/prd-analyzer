...
**总耗时（人工）**：一份中等复杂度 PRD 从接收到任务录入 Jira，通常需要 4-8 小时人力投入（包括评审会），且质量依赖 Tech Lead 个人经验。

### 1.2 核心痛点（量化）

| 痛点 | 数据来源 | 量化影响 |
|------|---------|---------|
| PRD 歧义导致需求变更 | McKinsey / Standish Group 研究 | 30-40% 的项目延期源于需求不清 |
| 需求中期变更成本 | NIST 研究 | 开发中修复成本是需求阶段的 10-100x |
| Tech Lead 时间分配 | 团队调研（典型样本） | 20-30% 时间用于非创造性的文档转化 |
| 任务粒度不一致 | Sprint velocity 波动 | 估算误差导致 Sprint 溢出率约 40% |

### 1.3 根本原因

PRD 是**非结构化自然语言**，而 Jira 任务需要**结构化可执行定义**。当前完全依赖人脑完成这一转化，存在：
- 认知负载高（理解 + 转化 + 决策同时进行）
- 标准不统一（不同 Tech Lead 拆解粒度、验收标准写法差异大）
- 歧义检测依赖经验（新手 Tech Lead 容易遗漏）
...
> "不是用 AI 替代人的判断，而是让 AI 处理机械转化，人专注于决策和审查。"
...
**改造后耗时（预估）**：人力从 4-8 小时降至 0.5-1 小时（审查 + 歧义答复）
...
| 维度 | 选型 | 理由 |
|------|------|------|
| 接口协议 | OpenAI 兼容（`/compatible-mode/v1`） | 无需更换 SDK，直接切换 `base_url` |
| 默认模型 | `deepseek-v4-flash` | 百炼免费额度可用，推理速度快，中文理解良好 |
| 高质量备选 | `qwen-plus` / `qwen-max` | 更强的结构化输出能力，付费账户可切换 |
| 结构化输出 | Tool Use（OpenAI function calling） | 强制 schema 合规，避免字符串解析 |

#### 为什么选 Function Calling（Tool Use）

| 方案 | 优点 | 缺点 | 决策 |
|------|------|------|------|
| Function calling | 强制结构化输出，Pydantic 直接验证 | 需要设计 tool schema | **选择** |
| 返回 JSON 字符串 | 简单 | 格式易出错，需手动解析，截断难以发现 | 否决 |
| 微调专属模型 | 定制化高 | 成本高，需大量训练数据 | 否决 |
| 规则引擎 + NLP | 可解释性强 | 无法处理非结构化输入的多样性 | 否决 |
...
- 支持深色/浅色主题切换（localStorage 持久化）
- 报告三标签页：需求（按类型分组 + 优先级徽章）/ 歧义（含澄清问题）/ 任务（含验收标准、风险、工时）
- 一键导出 JSON 和 Markdown

**评测框架（`evaluate.py`）**
- 对比 AI 输出与人工标注 ground truth，量化三项核心指标
- 默认使用 LLM-as-Judge 语义裁判（比 token-F1 更准确）
- 支持任意 PRD 和 GT 文件（`--prd`、`--gt` 参数），自动按命名约定推断 GT
- 保存带时间戳的评测报告（防文件覆盖）

**测试夹具（`tests/fixtures/`）**
- `complex_prd.md` + GT：购物车 / 结账 / 订单场景，23条需求（15功能 + 8非功能），13条歧义（5显性 + 8隐性）
- `points_prd.md` + GT：积分系统场景
- `tokenrouter_prd.md` + GT：真实开源项目 PRD（LLM API 网关），22条需求，8条歧义

### 3.2 MVP 不包含（Stage 2+）

- Jira/Linear API 直连（当前输出 JSON，手动导入）
- 历史 PRD 向量化存储（RAG）
- PM 实时歧义警告（Confluence 插件）
- 多用户权限体系 / 持久化存储（当前为内存存储）
...
Token-F1 匹配在中文场景下有固有局限：措辞不同但语义等价的句子 F1 趋近于 0。为此默认使用 LLM 语义裁判：
...
收益：准确率显著高于字符串匹配，能识别"措辞完全不同但意思相同"的情况。
